...
int receive_query(int sockfd, struct sockaddr_in servaddr) {
    ...
    while (unfinished && timeout_count < max_timeouts) {
        bzero(buffer, MAXMSG);
        n = recvfrom(sockfd, buffer, MAXMSG, 0, (struct sockaddr*)&recv_addr, &recv_len);
        
        if (n < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                timeout_count++;
                printf("Timeout waiting for RESPONSE or FINISH (%d/%d)\n", 
        ...
        /* Receive RESPONSE */
            ...
            if (parse_idx + sizeof(ip) + sizeof(port) + sizeof(name) <= n) {
    ...
    if (timeout_count >= max_timeouts) {
        printf("Maximum timeouts reached, giving up on receiving query\n");
        return -1;
    }
...
int p2p_client(unsigned int ip, unsigned short port, char *file_name) {
    ...
    while (recv_attempts < max_recv_attempts) {
        ...
        if (result < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                printf("Timeout receiving file size, retrying (%d/%d)\n", 
                       recv_attempts+1, max_recv_attempts);
                recv_attempts++;
                continue;
            } else {
            ...
            printf("Connection closed by peer while receiving file size\n");
            close(sock);
            return -1;
        } else {
            break; // 成功接收文件大小
    ...
    if (recv_attempts >= max_recv_attempts) {
        printf("Failed to receive file size after multiple attempts\n");
        close(sock);
        return -1;
    }
